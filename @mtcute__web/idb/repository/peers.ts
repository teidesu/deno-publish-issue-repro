...
import type { IdbStorageDriver } from "../driver.ts";
import { reqToPromise } from "../utils.ts";
const TABLE = 'peers';
// 
...
export class IdbPeersRepository implements IPeersRepository {
    ...
    async getById(id: number): Promise<IPeersRepository.PeerInfo | null> {
        ...
        if (!('isMin' in it))
            (it as any).isMin = false;
        return it;
    }
    async getByUsername(username: string): Promise<IPeersRepository.PeerInfo | null> {
        const it = await reqToPromise(this.os().index('by_username').get(username) as IDBRequest<IPeersRepository.PeerInfo>);
        // NB: older objects might not have isMin field
        if (!it || it.isMin)
            return null;
        return it;
