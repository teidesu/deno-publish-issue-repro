...
import { basename, joinPaths } from "../utils/fs.ts";
export async function readGramjsStoreSession(path: string, params?: {
    ...
    if ((typeof authKeyParsed !== 'object' || authKeyParsed === null)
...
export async function writeGramjsStoreSession(path: string, session: GramjsSession, params?: {
    /** filesystem implementation to use (defaults to `node:fs/promises`) */
    fs?: INodeFsLike;
}): Promise<void> {
    const fs: INodeFsLike = params?.fs ?? (await import('node:fs/promises') as unknown as INodeFsLike);
    ...
    await Promise.all([
        fs.writeFile(joinPaths(path, `${sessionName}%3AauthKey`), utf8.encoder.encode(JSON.stringify({
            type: 'Buffer',
