...
import type { TdataOptions } from "./tdata.ts";
import type { InputTdKeyData } from "./types.ts";
import { Long } from '@mtcute/core';
import { readStringSession } from '@mtcute/core/utils.js';
import { DC_MAPPING_PROD } from "../dcs.ts";
import { Tdata } from "./tdata.ts";
export async function convertFromTdata(tdata: Tdata | TdataOptions, accountIdx = 0): Promise<StringSessionData> {
    ...
    if (!authKey)
        throw new Error('Failed to find auth key');
    return {
        version: 3,
...
export async function convertToTdata(sessions: MaybeArray<StringSessionData | string>, tdata: Tdata | TdataOptions): Promise<void> {
