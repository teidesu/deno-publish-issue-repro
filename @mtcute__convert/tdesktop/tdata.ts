...
import type { IExtendedCryptoProvider } from "../utils/crypto.ts";
import type { INodeFsLike } from "../utils/fs.ts";
import type { InputTdKeyData, TdAuthKey, TdKeyData, TdMtpAuthorization } from "./types.ts";
...
import { getDefaultCryptoProvider } from "../utils/crypto.ts";
import { dirname, joinPaths } from "../utils/fs.ts";
import { readLong, readQByteArray } from "./qt-reader.ts";
import { writeLong, writeQByteArray } from "./qt-writer.ts";
...
export class Tdata {
    private constructor(readonly options: TdataOptions, readonly fs: INodeFsLike, readonly crypto: IExtendedCryptoProvider) { }
    ...
    static async create(options: TdataOptions & {
    ...
    async writeFile(filename: string, data: Uint8Array, mkdir = false): Promise<void> {
        ...
        await this.fs.writeFile(filename, u8.concat([TDF_MAGIC, version, data, await md5.digest()]));
    }
    async createLocalKey(salt: Uint8Array, passcode: string = this.options.passcode ?? ''): Promise<Uint8Array> {
        ...
        return this.crypto.pbkdf2(hash, salt, passcode === '' ? 1 : 100000, 256, 'sha512');
    }
    async decryptLocal(encrypted: Uint8Array, key: Uint8Array): Promise<Uint8Array> {
        ...
        const ige = createAesIgeForMessageOld(this.crypto, key, encryptedKey, false);
    ...
    async encryptLocal(data: Uint8Array, key: Uint8Array): Promise<Uint8Array> {
        ...
        const padding: Uint8Array = dataSize & 0x0F
    ...
    async readKeyData(): Promise<TdKeyData> {
        ...
        const infoDecrypted = await this.decryptLocal(infoEncrypted, keyInnerData);
    ...
    async readEncryptedFile(filename: string): Promise<[
        number,
        Uint8Array
    ]> {
    ...
    async writeEncryptedFile(filename: string, data: Uint8Array, mkdir = false): Promise<void> {
    ...
    async readMtpAuthorization(accountIdx: number = 0): Promise<TdMtpAuthorization> {
        const [, mtpData] = await this.readEncryptedFile(await this.computeDataNameKeyHex(accountIdx));
    ...
    async writeMtpAuthorization(auth: TdMtpAuthorization, accountIdx = 0): Promise<void> {
        ...
        await this.writeEncryptedFile(await this.computeDataNameKeyHex(accountIdx), file.result());
    }
    async writeEmptyMapFile(accountIdx: number): Promise<void> {
        ...
        await this.writeFile(joinPaths(await this.computeDataNameKeyHex(accountIdx), 'map'), writer.result(), true);
    }
}
