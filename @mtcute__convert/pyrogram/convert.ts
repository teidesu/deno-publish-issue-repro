...
import type { PyrogramSession } from "./types.ts";
import { readStringSession } from '@mtcute/core/utils.js';
import { DC_MAPPING_PROD, DC_MAPPING_TEST } from "../dcs.ts";
import { parsePyrogramSession } from "./parse.ts";
import { serializePyrogramSession } from "./serialize.ts";
...
export function convertToPyrogramSession(session: StringSessionData | string, params?: {
    apiId?: number;
}): string {
    if (typeof session === 'string') {
