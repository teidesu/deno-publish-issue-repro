...
import { tdFileId as td } from "./types.ts";
import { telegramRleDecode } from "./utils.ts";
    ...
    switch (variant) {
        ...
        case 1 /* THUMBNAIL */: {
            ...
            if (fileType < 0 || fileType >= td.FileType.Size) {
                throw new td.UnsupportedError(`Unsupported file type: ${fileType} (${base64.encode(reader.uint8View)})`);
            }
            const thumbnailType = reader.int();
            if (thumbnailType < 0 || thumbnailType > 255) {
                throw new td.InvalidFileIdError(`Wrong thumbnail type: ${thumbnailType} (${base64.encode(reader.uint8View)})`);
            }
            return {
                _: 'thumbnail',
                fileType,
        ...
        default:
            throw new td.UnsupportedError(`Unsupported photo size source ${variant} (${base64.encode(reader.uint8View)})`);
    }
}
        ...
        switch (fileType) {
            ...
            case td.FileType.Wallpaper: {
                ...
                switch (location.source._) {
                    case 'thumbnail':
                        if (location.source.fileType !== fileType
                    ...
                    case 'stickerSetThumbnailVersion':
                        if (fileType !== td.FileType.Thumbnail) {
                            throw new td.InvalidFileIdError('Invalid FileType in PhotoRemoteFileLocation StickerSetThumbnail');
                        }
                        break;
                }
                break;
...
export function parseFileId(fileId: string | Uint8Array): td.RawFullRemoteFileLocation {
    if (typeof fileId === 'string')
        fileId = base64.decode(fileId, true);
