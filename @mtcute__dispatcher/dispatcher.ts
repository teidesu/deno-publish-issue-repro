...
export class Dispatcher<State extends object = never> {
    ...
    private _errorHandler?: <T = {}>(err: Error, update: ParsedUpdate & T, state?: UpdateState<State>) => MaybePromise<boolean>;
    private _preUpdateHandler?: <T = {}>(update: ParsedUpdate & T, state?: UpdateState<State>) => MaybePromise<PropagationAction | void>;
    private _postUpdateHandler?: <T = {}>(handled: boolean, update: ParsedUpdate & T, state?: UpdateState<State>) => MaybePromise<void>;
    private _sceneTransitionHandler?: (update: SceneTransitionContext, state: UpdateState<State>) => MaybePromise<PropagationAction | void>;
     ...
     */
    static for<State extends object = never>(client: TelegramClient, ...args: [
        State
    ] extends [
        never
    ] ? [
    ...
    inject<Name extends keyof DispatcherDependencies>(name: Name | Partial<DispatcherDependencies>, value?: DispatcherDependencies[Name]): void {
        if (this._parent) {
    ...
    private async _dispatchUpdateNowImpl(update: ParsedUpdate, 
    // this is getting a bit crazy lol
    parsedState?: UpdateState<State> | null, parsedScene?: string | null, forceScene?: true, parsedContext?: UpdateContextType): Promise<boolean> {
        if (!this._client)
            return false;
