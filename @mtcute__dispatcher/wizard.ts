...
import type { DispatcherParams } from "./dispatcher.ts";
import type { UpdateFilter } from "./filters/index.ts";
import type { UpdateState } from "./state/update-state.ts";
...
export class WizardScene<State extends object> extends Dispatcher<State & WizardInternalState> {
    ...
    static onNthStep(step: number): UpdateFilter<any, {}, WizardInternalState> {
        if (step === 0) {
            return filters.or(filters.stateEmpty, filters.state<WizardInternalState>(it => it.$step === undefined || it.$step === 0));
        }
