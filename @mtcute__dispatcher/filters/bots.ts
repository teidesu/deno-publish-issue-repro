...
import type { MessageContext } from "../context/message.ts";
import type { Modify, UpdateFilter } from "./types.ts";
import { chat } from "./chat.ts";
import { and, or } from "./logic.ts";
/**
 ...
 * @param caseSensitive
 */
export function command(commands: MaybeArray<string | RegExp>, { prefixes = '/', caseSensitive = false, }: {
 ...
 * private messages.
 */
export const start: UpdateFilter<MessageContext | BusinessMessageContext, {
    chat: User;
    command: string[];
 ...
 */
export const startGroup: UpdateFilter<MessageContext | BusinessMessageContext, {
    chat: Modify<Chat, {
        chatType: 'group' | 'supergroup';
    }>;
    command: string[];
}, never> = and(or(chat('supergroup'), chat('group')), command('start'));
function deeplinkBase(base: UpdateFilter<MessageContext | BusinessMessageContext, {
    command: string[];
}>) {
    return (params: MaybeArray<string | RegExp>): UpdateFilter<MessageContext | BusinessMessageContext, {
        command: string[];
    }> => {
 ...
 */
export const deeplink: (params: MaybeArray<string | RegExp>) => UpdateFilter<MessageContext | BusinessMessageContext, {
    command: string[];
}> = deeplinkBase(start);
/**
 ...
 * meaning that the first group is available in `msg.command[2]`.
 */
export const deeplinkGroup: (params: MaybeArray<string | RegExp>) => UpdateFilter<MessageContext | BusinessMessageContext, {
    command: string[];
