import type { _RepliedMessageAssertionsByOrigin, Audio, Contact, Dice, Document, Game, Invoice, LiveLocation, Location, MaybeArray, Message, MessageAction, MessageMediaType, Peer, Photo, Poll, RepliedMessageInfo, RepliedMessageOrigin, Sticker, StickerSourceType, StickerType, Story, User, Venue, Video, Voice, WebPageMedia, } from '@mtcute/core';
import type { BusinessMessageContext } from "../context/business-message.ts";
import type { MessageContext } from "../index.ts";
import type { Modify, UpdateFilter } from "./types.ts";
 ...
 */
export const incoming: UpdateFilter<Message, {
    isOutgoing: false;
}> = msg => !msg.isOutgoing;
/**
 ...
 */
export const outgoing: UpdateFilter<Message, {
    isOutgoing: true;
}> = msg => msg.isOutgoing;
/**
 * Filter for scheduled messages
 */
export const scheduled: UpdateFilter<Message, {
    isScheduled: true;
}> = msg => msg.isScheduled;
/**
 * Filter messages that are replies to some other message
 */
export const reply: UpdateFilter<Message, {
    replyToMessage: RepliedMessageInfo;
}> = msg => msg.replyToMessage !== null;
/**
 * Filter messages that are replies with the given origin type
 */
export function replyOrigin<T extends RepliedMessageOrigin>(origin: T): UpdateFilter<Message, {
 ...
 */
export const media: UpdateFilter<Message, {
    media: Exclude<Message['media'], null>;
}> = msg => msg.media !== null;
/**
 * Filter messages containing media of given type
 */
export function mediaOf<T extends MessageMediaType>(type: T): UpdateFilter<Message, {
...
export const photo: UpdateFilter<Message, {
    media: Photo;
}> = mediaOf('photo');
/** Filter messages containing a dice */
export const dice: UpdateFilter<Message, {
    media: Dice;
}> = mediaOf('dice');
/** Filter messages containing a contact */
export const contact: UpdateFilter<Message, {
    media: Contact;
}> = mediaOf('contact');
/** Filter messages containing an audio file */
export const audio: UpdateFilter<Message, {
    media: Audio;
}> = mediaOf('audio');
/** Filter messages containing a voice message (audio-only) */
export const voice: UpdateFilter<Message, {
    media: Voice;
}> = mediaOf('voice');
/** Filter messages containing a sticker */
export const sticker: UpdateFilter<Message, {
    media: Sticker;
}> = mediaOf('sticker');
/** Filter messages containing a document (a file) */
export const document: UpdateFilter<Message, {
    media: Document;
}> = mediaOf('document');
/** Filter messages containing any video (videos, round messages and animations) */
export const anyVideo: UpdateFilter<Message, {
    media: Video;
}> = mediaOf('video');
/** Filter messages containing a static location */
export const location: UpdateFilter<Message, {
    media: Location;
}> = mediaOf('location');
/** Filter messages containing a live location */
export const liveLocation: UpdateFilter<Message, {
    media: LiveLocation;
}> = mediaOf('live_location');
/** Filter messages containing a game */
export const game: UpdateFilter<Message, {
    media: Game;
}> = mediaOf('game');
/** Filter messages containing a web page */
export const webpage: UpdateFilter<Message, {
    media: WebPageMedia;
}> = mediaOf('webpage');
/** Filter messages containing a venue */
export const venue: UpdateFilter<Message, {
    media: Venue;
}> = mediaOf('venue');
/** Filter messages containing a poll */
export const poll: UpdateFilter<Message, {
    media: Poll;
}> = mediaOf('poll');
/** Filter messages containing an invoice */
export const invoice: UpdateFilter<Message, {
    media: Invoice;
}> = mediaOf('invoice');
/** Filter messages containing a story */
export const story: UpdateFilter<Message, {
    media: Story;
}> = mediaOf('story');
/**
 * Filter messages containing any location (live or static).
 */
export const anyLocation: UpdateFilter<Message, {
    media: Location;
}> = msg => msg.media instanceof RawLocation;
/**
 ...
 */
export const anyDocument: UpdateFilter<Message, {
    media: RawDocument;
}> = msg => msg.media instanceof RawDocument;
/**
 * Filter messages containing a simple video.
 *
 * This does not include round messages and animations
 */
export const video: UpdateFilter<Message, {
    media: Modify<Video, {
        isRound: false;
        isAnimation: false;
    }>;
 ...
 */
export const animation: UpdateFilter<Message, {
    media: Modify<Video, {
        isRound: false;
        isAnimation: true;
    }>;
 ...
 */
export const roundMessage: UpdateFilter<Message, {
    media: Modify<Video, {
        isRound: true;
        isAnimation: false;
    }>;
 ...
 */
export function stickerByType(type: StickerType): UpdateFilter<Message, {
    media: Sticker;
}> {
 ...
 */
export function stickerBySourceType(type: StickerSourceType): UpdateFilter<Message, {
    media: Sticker;
}> {
 ...
 */
export const text: UpdateFilter<Message, {
    media: null;
    isService: false;
 ...
 */
export const service: UpdateFilter<Message, {
    isService: true;
}> = msg => msg.isService;
/**
 * Filter service messages by action type
 */
export function action<T extends Exclude<MessageAction, null>['type']>(type: MaybeArray<T>): UpdateFilter<Message, {
...
export function sender<T extends Message['sender']['type']>(type: T): UpdateFilter<Message, {
 ...
 */
export function replyTo<Mod, State extends object>(filter?: UpdateFilter<Message, Mod, State>): UpdateFilter<MessageContext | BusinessMessageContext, {
 ...
 * and make it available to further filters, as well as the handler itself.
 */
export function withCompleteChat<Mod, State extends object>(filter?: UpdateFilter<MessageContext, Mod, State>): UpdateFilter<MessageContext, Mod, State> {
    return async (msg, state) => {
        try {
