 ...
 * @returns
 */
export function every<Mod, State extends object>(filter: UpdateFilter<Message, Mod, State>): UpdateFilter<MessageContext | BusinessMessageContext, Mod & {
 ...
 * @returns
 */
export function some<State extends object>(filter: UpdateFilter<Message, any, State>): UpdateFilter<MessageContext | BusinessMessageContext, {}, State> {
    return (ctx, state) => {
        let i = 0;
        const upds = ctx.messages;
        const max = upds.length;
