...
import type { StateService } from "./service.ts";
...
export class UpdateState<State extends object> {
     ...
     */
    async merge(state: Partial<State>, params: {
     ...
     * Enter some scene
     */
    async enter<SceneState extends object, Scene extends Dispatcher<SceneState>>(scene: Scene, params?: {
        /**
     ...
     */
    async rateLimit(key: string, limit: number, window: number): Promise<[
        number,
        number
    ]> {
     ...
     *   unix time in ms when the user can try again
     */
    async throttle(key: string, limit: number, window: number): Promise<[
        number,
        number
    ]> {
        try {
