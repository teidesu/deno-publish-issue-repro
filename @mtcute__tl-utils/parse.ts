import type { TlArgument, TlEntry } from "./types.ts";
import { computeConstructorIdFromEntry } from "./ctor-id.ts";
import { TL_PRIMITIVES } from "./types.ts";
import { parseArgumentType, parseTdlibStyleComment } from "./utils.ts";
 ...
 */
export function parseTlToEntries(tl: string, params: {
    /**
    ...
    lines.forEach((line, idx) => {
        ...
        const argsParsed = args && !args.match(/\[ [a-z]+ \]/i)
            ? args
                .trim()
                .split(' ')
                .map(j => j.split(':'))
        ...
        entry.arguments.forEach((arg) => {
            ...
            if (arg.typeModifiers?.isBareUnion) {
                if (!(type in unions)) {
                    ...
                    handleError(new Error(`Union ${type} has more than one entry, cannot use it like %${type} (found in ${entry.name}#${arg.name})`), entryIdx);
                }
                else {
