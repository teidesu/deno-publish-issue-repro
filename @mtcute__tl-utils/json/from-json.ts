...
import type { TlParamJson } from "./types.ts";
import { TL_PRIMITIVES } from "../types.ts";
import { parseArgumentType } from "../utils.ts";
import { parseTlSchemaFromJson } from "./types.ts";
...
export function parseTlEntriesFromJson(json: object, params?: {
    /**
    ...
    schema.constructors.forEach((c) => {
        if (!keepPrimitives && (c.predicate in TL_PRIMITIVES || c.type in TL_PRIMITIVES))
            return;
        const entry: TlEntry = {
            id: Number(c.id) >>> 0,
            kind: 'class',
