import type { TlEntry, TlTypeModifiers } from "./types.ts";
/**
 ...
 */
export function splitNameToNamespace(name: string): [
    string | null,
    string
] {
    const s = name.split('.');
...
export function stringifyArgumentType(type: string, modifiers?: TlTypeModifiers): string {
    if (!modifiers)
        return type;
    let ret = type;
    if (modifiers.isBareUnion)
        ret = `%${ret}`;
    if (modifiers.isVector)
        ret = `Vector<${ret}>`;
    else if (modifiers.isBareVector)
        ret = `vector<${ret}>`;
    if (modifiers.predicate)
        ret = `${modifiers.predicate}?${ret}`;
    return ret;
}
export function parseArgumentType(type: string): [
    string,
    TlTypeModifiers
] {
