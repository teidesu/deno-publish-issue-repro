import type { TlEntry, TlErrors, TlFullSchema, TlTypeModifiers } from "../types.ts";
import { groupTlEntriesByNamespace, splitNameToNamespace } from "../utils.ts";
import { generateCodeForErrors } from "./errors.ts";
import { camelToPascal, indent, jsComment, snakeToCamel } from "./utils.ts";
/**
...
function fullTypeName(type: string, baseNamespace: string, { namespace = true, method = false, link = false, typeModifiers, }: {
    namespace?: boolean;
    method?: boolean;
    link?: boolean;
    ...
    if (type in PRIMITIVE_TO_TS)
        return PRIMITIVE_TO_TS[type];
 ...
 */
export function generateTypescriptDefinitionsForTlEntry(entry: TlEntry, params?: {
 ...
 */
export function generateTypescriptDefinitionsForTlSchema(schema: TlFullSchema, params?: {
    /** Layer of the schema */
    layer?: number;
    ...
    const { layer = 0, namespace = 'tl', errors, skipLongImport = false, onlyTypings = false, extends: extendsNamespace, extendsSchema, } = params ?? {};
...
`;
    ...
    schema.entries.forEach((entry) => {
        ...
        ts
            += `${indent(8, `| ${fullTypeName(entry.name, `${namespace}.`, {
                method: entry.kind === 'method',
            })}`)}\n`;
    });
    if (extendsNamespace) {
