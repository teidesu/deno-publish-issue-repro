import type { TlArgument, TlEntry, TlFullSchema } from "./types.ts";
...
export function mergeTlEntries(entries: TlEntry[]): TlEntry | string {
    ...
    for (let i = 1; i < entries.length; i++) {
        ...
        if (result.kind !== entry.kind)
            return 'basic info mismatch - kind';
        if (result.name !== entry.name)
            return 'basic info mismatch - name';
        if (result.type !== entry.type)
            return 'basic info mismatch - type';
        if (result.id !== ctorId)
            return 'basic info mismatch - id';
 ...
 */
export async function mergeTlSchemas(schemas: TlFullSchema[], onConflict: (options: (TlEntry | undefined)[], reason: string) => TlEntry | undefined | Promise<TlEntry | undefined>): Promise<TlFullSchema> {
