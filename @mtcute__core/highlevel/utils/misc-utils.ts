import type { ArrayPaginated, ArrayPaginatedWithMeta, ArrayWithTotal, MaybeDynamic, Message } from "../types/index.ts";
...
export async function resolveMaybeDynamic<T>(val: MaybeDynamic<T>): Promise<T> {
    ...
    return typeof val === 'function' ? await (val as () => Promise<T>)() : await val;
}
...
export function makeArrayPaginatedWithMeta<T, Offset, Meta>(arr: T[], total: number, meta: Meta, next?: Offset): ArrayPaginatedWithMeta<T, Offset, Meta> {
