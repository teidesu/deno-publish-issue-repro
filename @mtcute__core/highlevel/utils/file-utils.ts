...
export function isProbablyPlainText(buf: Uint8Array): boolean {
    return !buf.some(it => !(((it >= 0x20 && it < 0x7F) // printable ascii range
        || it === 0x0D // CR
        || it === 0x0A // LF
        || it === 0x09) // Tab
    ));
...
function JPEG_HEADER() {
    return hex.decode('ffd8ffe000104a46494600010100000100010000ffdb004300281c1e231e1928'
...
export function svgPathToFile(path: string, size?: tl.RawDocumentAttributeImageSize): Uint8Array {
    return utf8.encoder.encode('<?xml version="1.0" encoding="utf-8"?>'
