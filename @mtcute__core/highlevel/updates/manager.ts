...
import type { BaseTelegramClient } from "../base.ts";
import type { PendingUpdate, PendingUpdateContainer, UpdatesManagerParams } from "./types.ts";
...
import { EarlyTimer, getBarePeerId, getMarkedPeerId, isInputPeerChannel, isInputPeerUser, parseMarkedPeerId, SortedArray, toggleChannelIdMark, toInputChannel, toInputUser, } from "../../utils/index.ts";
import { _getChannelsBatched, _getUsersBatched } from "../methods/chats/batched-queries.ts";
import { PeersIndex } from "../types/peers/peers-index.ts";
import { RawUpdateInfo } from "./types.ts";
import { createDummyUpdatesContainer, extractChannelIdFromUpdate, isMessageEmpty, messageToUpdate, toPendingUpdate, } from "./utils.ts";
...
export class UpdatesManager {
    ...
    constructor(readonly client: BaseTelegramClient, readonly params: UpdatesManagerParams = {}) {
    ...
    handleUpdate(update: tl.TypeUpdates): void {
        this.log.debug('received %s, queueing for processing. containers queue size: %d', update._, this.pendingUpdateContainers.length);
    ...
    async _fetchUpdatesState(): Promise<void> {
        ...
        try {
            ...
            log.debug('updates.getState returned state: pts=%d, qts=%d, date=%d, seq=%d', fetchedState.pts, fetchedState.qts, fetchedState.date, fetchedState.seq);
    ...
    async _loadUpdatesStorage(): Promise<void> {
        ...
        if (storedState) {
            ...
            this.log.debug('loaded stored state: pts=%d, qts=%d, date=%d, seq=%d', storedState[0], storedState[1], storedState[2], storedState[3]);
        }
    ...
    _addToNoDispatchIndex(updates?: tl.TypeUpdates): void {
        ...
        const addUpdate = (upd: tl.TypeUpdate) => {
            ...
            if (pts) {
                ...
                if (!set)
                    noDispatchPts.set(channelId, new Set([pts]));
                else
                    set.add(pts);
            }
            ...
            switch (upd._) {
                ...
                case 'updateBotNewBusinessMessage': {
                    ...
                    if (!set)
                        noDispatchMsg.set(channelId, new Set([upd.message.id]));
                    else
                        set.add(upd.message.id);
                    break;
                }
            }
        };
        switch (updates._) {
            ...
            case 'updateShortSentMessage': {
                ...
                if (!set)
                    noDispatchMsg.set(0, new Set([updates.id]));
                else
                    set.add(updates.id);
                set = noDispatchPts.get(0);
                if (!set)
                    noDispatchPts.set(0, new Set([updates.pts]));
                else
                    set.add(updates.pts);
                break;
            }
    ...
    async _fetchMissingPeers(upd: tl.TypeUpdate, peers: PeersIndex, fromDifference = false): Promise<Set<number>> {
        ...
        async function fetchPeer(peer?: tl.TypePeer | number, allowZeroHash = false) {
            ...
            if (fromIndex && !(fromIndex as Extract<typeof fromIndex, {
                min?: unknown;
            }>).min)
                return true;
        ...
        switch (upd._) {
            ...
            case 'updateEditChannelMessage': {
                ...
                if (!(await fetchPeer(msg.peerId, true)))
                    return missing;
                if (!(await fetchPeer(msg.fromId)))
                    return missing;
                if (msg.replyTo) {
                ...
                if (msg._ !== 'messageService') {
                    ...
                    if (!(await fetchPeer(msg.viaBotId)))
                        return missing;
                    if (msg.entities) {
                        for (const ent of msg.entities) {
                            if (ent._ === 'messageEntityMentionName') {
                                if (!(await fetchPeer(ent.userId)))
                                    return missing;
                            }
                        }
                    }
                    ...
                    switch (msg.action._) {
                        ...
                        case 'messageActionChatDeleteUser':
                            if (!(await fetchPeer(msg.action.userId)))
                                return missing;
                            break;
                        ...
                        case 'messageActionChannelMigrateFrom':
                            if (!(await fetchPeer(-msg.action.chatId)))
                                return missing;
                            break;
                        case 'messageActionGeoProximityReached':
                            if (!(await fetchPeer(msg.action.fromId)))
                                return missing;
                            if (!(await fetchPeer(msg.action.toId)))
                                return missing;
                            break;
                    }
                }
    ...
    async _fetchChannelDifference(channelId: number, fallbackPts?: number): Promise<boolean> {
        ...
        for (;;) {
            ...
            if (diff._ === 'updates.channelDifferenceTooLong') {
                ...
                if (this.params.onChannelTooLong) {
                    ...
                    log.warn('getChannelDifference (cid = %d) returned channelDifferenceTooLong. new pts: %d, recent msgs: %d', channelId, pts, diff.messages.length);
                    diff.messages.forEach((message) => {
                        log.debug('processing message %d (%s) from TooLong diff for channel %d', message.id, message._, channelId);
            ...
            log.debug('getChannelDifference (cid = %d) returned %d messages, %d updates. new pts: %d, final: %b', channelId, diff.newMessages.length, diff.otherUpdates.length, diff.pts, diff.final);
            ...
            diff.otherUpdates.forEach((upd) => {
                ...
                log.debug('processing %s from diff for channel %d, pts_before: %d, pts: %d', upd._, channelId, parsed.ptsBefore, parsed.pts);
        ...
        if (lastTimeout !== 0 && channelsOpened.has(channelId)) {
            ...
            channelDiffTimeouts.set(channelId, timers.setTimeout(() => this._fetchChannelDifferenceViaUpdate(channelId), lastTimeout * 1000));
        }
        return true;
    }
    ...
    _fetchChannelDifferenceViaUpdate(channelId: number, pts?: number): void {
        this.handleUpdate(createDummyUpdatesContainer([
            {
    ...
    async _fetchDifference(requestedDiff: Map<number, Promise<void>>): Promise<void> {
        ...
        log.debug('updates.getDifference returned %d messages, %d updates. new pts: %d, qts: %d, seq: %d, final: %b', diff.newMessages.length, diff.otherUpdates.length, fetchedState.pts, fetchedState.qts, fetchedState.seq, diff._ === 'updates.difference');
        const peers = PeersIndex.from(diff);
        diff.newMessages.forEach((message) => {
        ...
        diff.otherUpdates.forEach((upd) => {
            if (upd._ === 'updateChannelTooLong') {
                log.debug('received updateChannelTooLong for channel %d in common diff (pts = %d), fetching diff', upd.channelId, upd.pts);
            ...
            log.debug('received %s from common diff, cid: %d, pts_before: %d, pts: %d, qts_before: %d', upd._, parsed.channelId, parsed.ptsBefore, parsed.pts, parsed.qtsBefore);
        });
        this.pts = fetchedState.pts;
        this.qts = fetchedState.qts;
        this.seq = fetchedState.seq;
    ...
    _fetchDifferenceLater(requestedDiff: Map<number, Promise<void>>): void {
        if (!requestedDiff.has(0)) {
            requestedDiff.set(0, this._fetchDifference(requestedDiff)
                .catch((err) => {
    ...
    async _onUpdate(pending: PendingUpdate, requestedDiff: Map<number, Promise<void>>, postponed = false, unordered = false): Promise<void> {
        const { client, log } = this;
        ...
        if (pending.peers.hasMin || pending.peers.empty) {
            ...
            if (!pending.fromDifference && missing.size) {
                log.debug('fetching difference because some peers were min (%J) and not cached for %s (pts = %d, cid = %d)', missing, upd._, pending.pts, pending.channelId);
            ...
            if (missing.size) {
                log.debug('peers still missing after fetching difference: %J for %s (pts = %d, cid = %d)', missing, upd._, pending.pts, pending.channelId);
            }
        }
        ...
        if (!unordered) {
            ...
            if (pending.pts) {
                ...
                if (localPts && pending.ptsBefore !== localPts) {
                    log.warn('pts_before does not match local_pts for %s (cid = %d, pts_before = %d, pts = %d, local_pts = %d)', upd._, pending.channelId, pending.ptsBefore, pending.pts, localPts);
                }
                log.debug('applying new pts (cid = %d) because received %s: %d -> %d (before: %d, count: %d) (postponed = %s)', pending.channelId, upd._, localPts, pending.pts, pending.ptsBefore, pending.pts - pending.ptsBefore!, postponed);
            ...
            if (pending.qtsBefore) {
                log.debug('applying new qts because received %s: %d -> %d (postponed = %s)', upd._, this.qts, pending.qtsBefore + 1, postponed);
        ...
        if (missing?.size) {
            if (client.storage.self.getCached()?.isBot) {
                this.log.warn('missing peers (%J) after getDifference for %s (pts = %d, cid = %d)', missing, upd._, pending.pts, pending.channelId);
            }
            else {
        ...
        if (this.noDispatchEnabled) {
            ...
            const msgId = upd._ === 'updateNewMessage'
    ...
    async _loop(): Promise<void> {
        const { log, client, cpts, cptsMod, pendingUpdateContainers, pendingPtsUpdates, pendingPtsUpdatesPostponed, pendingQtsUpdates, pendingQtsUpdatesPostponed, pendingUnorderedUpdates, updatesLoopCv, postponedTimer, } = this;
        ...
        try {
            ...
            while (this.updatesLoopActive) {
                if (!(pendingUpdateContainers.length
                ...
                if (!this.updatesLoopActive)
                    break;
                log.debug('updates loop tick. pending containers: %d, pts: %d, pts_postponed: %d, qts: %d, qts_postponed: %d, unordered: %d', pendingUpdateContainers.length, pendingPtsUpdates.length, pendingPtsUpdatesPostponed.length, pendingQtsUpdates.length, pendingQtsUpdatesPostponed.length, pendingUnorderedUpdates.length);
                    ...
                    switch (upd._) {
                        ...
                        case 'updates': {
                            if (seqStart !== 0) {
                                ...
                                log.debug('received seq-ordered %s (seq_start = %d, seq_end = %d, size = %d)', upd._, seqStart, seqEnd, upd.updates.length);
                                if (nextLocalSeq > seqStart) {
                                    log.debug('ignoring updates group because already applied (by seq: exp %d, got %d)', nextLocalSeq, seqStart);
                                    // "the updates were already applied, and must be ignored"
                                    continue;
                                }
                                if (nextLocalSeq < seqStart) {
                                    log.debug('fetching difference because gap detected (by seq: exp %d, got %d)', nextLocalSeq, seqStart);
                            ...
                            for (const update of upd.updates) {
                                switch (update._) {
                                    case 'updateChannelTooLong':
                                        log.debug('received updateChannelTooLong for channel %d (pts = %d) in container, fetching diff', update.channelId, update.pts);
                                    ...
                                    case 'updatePtsChanged':
                                        ...
                                        if (client.mt.network.getPoolSize('main') > 1) {
                                            // highload bot
                                            log.debug('updatePtsChanged received, resetting pts to 1 and fetching difference');
                                            this.pts = 1;
                                            this._fetchDifferenceLater(requestedDiff);
                                        }
                                        else {
                ...
                while (pendingPtsUpdates.length) {
                    ...
                    if (localPts) {
                        ...
                        if (diff > 0 && !isPtsDrop) {
                            ...
                            log.debug('ignoring %s (cid = %d) because already applied (by pts: exp %d, got %d)', upd._, pending.channelId, localPts, pending.ptsBefore);
                            continue;
                        }
                        if (diff < 0) {
                            ...
                            if (diff > -3) {
                                log.debug('postponing %s for 0.5s (cid = %d) because small gap detected (by pts: exp %d, got %d, diff=%d)', upd._, pending.channelId, localPts, pending.ptsBefore, diff);
                                ...
                                log.debug('fetching difference after %s (cid = %d) because pts gap detected (by pts: exp %d, got %d, diff=%d)', upd._, pending.channelId, localPts, pending.ptsBefore, diff);
                                if (pending.channelId) {
                                ...
                                log.debug('skipping all updates because pts gap is too big (by pts: exp %d, got %d, diff=%d)', localPts, pending.ptsBefore, diff);
                                if (pending.channelId) {
                ...
                for (let i = 0; i < pendingPtsUpdatesPostponed.raw.length; i++) {
                    ...
                    if (localPts > pending.ptsBefore!) {
                        // "the update was already applied, and must be ignored"
                        log.debug('ignoring postponed %s (cid = %d) because already applied (by pts: exp %d, got %d)', upd._, pending.channelId, localPts, pending.ptsBefore);
                    ...
                    if (localPts < pending.ptsBefore!) {
                        ...
                        if (now < pending.timeout!) {
                            log.debug('postponed %s (cid = %d) is still waiting (%dms left) (current pts %d, need %d)', upd._, pending.channelId, pending.timeout! - now, localPts, pending.ptsBefore);
                        }
                        else {
                            log.debug("gap for postponed %s (cid = %d) wasn't filled, fetching diff (current pts %d, need %d)", upd._, pending.channelId, localPts, pending.ptsBefore);
                            pendingPtsUpdatesPostponed.removeIndex(i);
                            i--;
                            if (pending.channelId) {
                                this._fetchChannelDifferenceLater(requestedDiff, pending.channelId);
                ...
                while (pendingQtsUpdates.length) {
                    ...
                    if (diff > 0 && !isQtsDrop) {
                        // "the update was already applied, and must be ignored"
                        log.debug('ignoring %s because already applied (by qts: exp %d, got %d)', upd._, this.qts!, pending.qtsBefore);
                        continue;
                    }
                    if (this.qts! < pending.qtsBefore!) {
                        ...
                        // otherwise, call getDifference
                        if (diff > -3) {
                            log.debug('postponing %s for 0.5s because small gap detected (by qts: exp %d, got %d, diff=%d)', upd._, this.qts!, pending.qtsBefore, diff);
                            pending.timeout = performance.now() + 500;
                            pendingQtsUpdatesPostponed.insert(pending);
                            postponedTimer.emitBefore(pending.timeout);
                        }
                        else {
                            log.debug('fetching difference after %s because qts gap detected (by qts: exp %d, got %d, diff=%d)', upd._, this.qts!, pending.qtsBefore, diff);
                            this._fetchDifferenceLater(requestedDiff);
                        }
                        continue;
                    }
                ...
                for (let i = 0; i < pendingQtsUpdatesPostponed.raw.length; i++) {
                    ...
                    if (this.qts! > pending.qtsBefore!) {
                        // "the update was already applied, and must be ignored"
                        log.debug('ignoring postponed %s because already applied (by qts: exp %d, got %d)', upd._, this.qts!, pending.qtsBefore);
                        continue;
                    }
                    if (this.qts! < pending.qtsBefore!) {
                        // "there's an update gap that must be filled"
                        ...
                        const now = performance.now();
                        if (now < pending.timeout!) {
                            log.debug('postponed %s is still waiting (%dms left) (current qts %d, need %d)', upd._, pending.timeout! - now, this.qts!, pending.qtsBefore);
                        }
                        else {
                            log.debug("gap for postponed %s wasn't filled, fetching diff (current qts %d, need %d)", upd._, this.qts!, pending.qtsBefore);
                ...
                while (requestedDiff.size) {
                    log.debug('waiting for %d pending diffs before processing unordered: %J', requestedDiff.size, requestedDiff.keys());
                    await Promise.all([...requestedDiff.values()]);
                    // diff results may as well contain new diffs to be requested
                    log.debug('pending diffs awaited, new diffs requested: %d (%J)', requestedDiff.size, requestedDiff.keys());
                }
                ...
                while (requestedDiff.size) {
                    log.debug('waiting for %d pending diffs after processing unordered: %J', requestedDiff.size, requestedDiff.keys());
                    await Promise.all([...requestedDiff.values()]);
                    // diff results may as well contain new diffs to be requested
                    log.debug('pending diffs awaited, new diffs requested: %d (%j)', requestedDiff.size, requestedDiff.keys());
                }
