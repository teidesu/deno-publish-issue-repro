...
import type { MtClientOptions } from "../network/client.ts";
import type { ConnectionKind, RpcCallOptions } from "../network/network-manager.ts";
import type { StorageManagerExtraOptions } from "../storage/storage.ts";
import type { ICorePlatform } from "../types/platform.ts";
import type { MustEqual } from "../types/utils.ts";
import type { ICryptoProvider, InputStringSessionData, Logger, } from "../utils/index.ts";
import type { ConnectionState, ITelegramClient } from "./client.types.ts";
import type { ITelegramStorageProvider } from "./storage/provider.ts";
import type { TelegramStorageManagerExtraOptions } from "./storage/storage.ts";
import type { RawUpdateInfo, UpdatesManagerParams } from "./updates/types.ts";
import { Emitter, unknownToError } from '@fuman/utils';
import { tl } from '@mtcute/tl';
import { MtClient } from "../network/client.ts";
import { reportUnknownError } from "../utils/error-reporting.ts";
import { asyncResettable, computeNewPasswordHash, computeSrpParams, isTlRpcError, readStringSession, writeStringSession, } from "../utils/index.ts";
import { LogManager } from "../utils/logger.ts";
import { AppConfigManager } from "./managers/app-config-manager.ts";
import { TimersManager } from "./managers/timers.ts";
import { TelegramStorageManager } from "./storage/storage.ts";
import { UpdatesManager } from "./updates/manager.ts";
...
export class BaseTelegramClient implements ITelegramClient {
     ...
     */
    async call<T extends tl.RpcMethod>(message: MustEqual<T, tl.RpcMethod>, params?: RpcCallOptions): Promise<tl.RpcCallReturn[T['_']]> {
        if (!this._connected) {
