...
import type { HistoryReadUpdate, ParsedUpdate } from "./updates/index.ts";
import type { ParametersSkip2 } from "./utils.ts";
import { AsyncLock, Deferred, Deque, timers, unknownToError } from '@fuman/utils';
import { MtArgumentError, MtTimeoutError } from "../../types/errors.ts";
...
import { resolvePeer } from "../methods/users/resolve-peer.ts";
...
export class Conversation {
    ...
    constructor(readonly client: ITelegramClient, readonly chat: InputPeerLike) {
     ...
     */
    markRead({ message, clearMentions = true, }: {
        message?: number | null;
     ...
     */
    waitForNewMessage(filter?: (msg: Message) => MaybePromise<boolean>, timeout: number | null = 15000): Promise<Message> {
        if (!this._started) {
            throw new MtArgumentError("Conversation hasn't started yet");
        }
     ...
     * @param params
     */
    async waitForEdit(filter?: (msg: Message) => MaybePromise<boolean>, params?: {
        /**
         * Message for which to wait for reply for.
         *
    ...
    async waitForRead(message?: number, timeout: number | null = 15000): Promise<void> {
        ...
        const [dialog] = await getPeerDialogs(this.client, this._inputPeer);
        if (!dialog || dialog.lastRead >= msgId)
            return;
        ...
        void this._lock.acquire().then(async () => {
            try {
                if (!it.check || (await it.check(msg))) {
                    if (it.timeout)
                        timers.clearTimeout(it.timeout);
