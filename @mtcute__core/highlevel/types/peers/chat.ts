...
import { MessageEntity } from "../messages/message-entity.ts";
import { EmojiStatus } from "../reactions/emoji-status.ts";
import { ChatColors } from "./chat-colors.ts";
import { ChatPermissions } from "./chat-permissions.ts";
import { ChatPhoto } from "./chat-photo.ts";
/**
 * Chat type. Can be:
...
export class Chat {
    ...
    get color(): ChatColors {
        return new ChatColors(this.raw.id, this.raw._ === 'channel' && this.raw.color?._ === 'peerColor' ? this.raw.color : undefined);
    }
    /**
    ...
    get profileColors(): ChatColors {
        return new ChatColors(this.raw.id, this.raw._ === 'channel' && this.raw.profileColor?._ === 'peerColor' ? this.raw.profileColor : undefined);
    }
    ...
    get subscriptionUntilDate(): Date | null {
        if (this.raw._ !== 'channel' || !this.raw.subscriptionUntilDate)
            return null;
    ...
    get bannedUntilDate(): Date | null {
        if (this.raw._ !== 'channelForbidden' || !this.raw.untilDate)
            return null;
