...
export class PeersIndex {
    constructor(readonly users: Map<number, tl.TypeUser> = new Map(), readonly chats: Map<number, tl.TypeChat> = new Map()) { }
    hasMin = false;
    static from(obj: {
        users?: tl.TypeUser[];
        chats?: tl.TypeChat[];
        ...
        obj.chats?.forEach((chat) => {
            ...
            if ((chat as Exclude<typeof chat, tl.RawChatEmpty | tl.RawChat | tl.RawChatForbidden | tl.RawChannelForbidden>).min) {
                index.hasMin = true;
            }
        });
        return index;
