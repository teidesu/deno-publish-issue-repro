...
import { StickerSet } from "../misc/sticker-set.ts";
import { PeerStories } from "../stories/peer-stories.ts";
import { BotInfo } from "./bot-info.ts";
import { BotVerification } from "./bot-verification.ts";
import { ChatInviteLink } from "./chat-invite-link.ts";
import { ChatLocation } from "./chat-location.ts";
...
export class FullChat extends Chat {
    ...
    get stickerSet(): StickerSet | null {
        if (this.full?._ !== 'channelFull' || !this.full.stickerset)
            return null;
    ...
    get emojiSet(): StickerSet | null {
        if (this.full?._ !== 'channelFull' || !this.full.emojiset)
            return null;
    ...
    get boostsApplied(): number {
        if (!this.full || this.full._ !== 'channelFull')
            return 0;
    ...
    get migratedFrom(): {
        ...
        if (this.full._ !== 'channelFull')
            return null;
        if (!this.full.migratedFromChatId || !this.full.migratedFromMaxId)
            return null;
        return {
    ...
    get monoforumLinkedChat(): Chat | null {
        if (this.raw._ !== 'channel')
            return null;
        if (!this.raw.linkedMonoforumId)
            return null;
    ...
    get slowmodeNextSendDate(): Date | null {
        if (this.full._ !== 'channelFull' || !this.full.slowmodeNextSendDate)
            return null;
    ...
    get recentRequesters(): User[] {
        if (this.full._ !== 'channelFull' || !this.full.recentRequesters)
            return [];
    ...
    get stories(): PeerStories | null {
        if (this.full._ !== 'channelFull' || !this.full.stories)
            return null;
    ...
    get botVerification(): BotVerification | null {
        if (this.full._ !== 'channelFull' || !this.full.botVerification)
            return null;
