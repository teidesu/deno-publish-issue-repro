...
import type { ReplyMarkup } from "../bots/keyboards/index.ts";
import type { TextWithEntities } from "../misc/index.ts";
import type { Chat } from "../peers/chat.ts";
import type { Peer } from "../peers/peer.ts";
import type { PeersIndex } from "../peers/peers-index.ts";
import type { MessageAction } from "./message-action.ts";
import type { MessageMedia } from "./message-media.ts";
import { MtArgumentError } from "../../../types/errors.ts";
import { assertNever } from "../../../types/utils.ts";
import { getMarkedPeerId, toggleChannelIdMark } from "../../../utils/peer-utils.ts";
...
import { BotKeyboard } from "../bots/keyboards/index.ts";
import { parsePeer } from "../peers/peer.ts";
import { User } from "../peers/user.ts";
import { FactCheck } from "./fact-check.ts";
import { _messageActionFromTl } from "./message-action.ts";
import { MessageEntity } from "./message-entity.ts";
import { MessageForwardInfo } from "./message-forward.ts";
import { _messageMediaFromTl } from "./message-media.ts";
import { MessageReactions } from "./message-reactions.ts";
import { MessageRepliesInfo } from "./message-replies.ts";
import { RepliedMessageInfo } from "./replied-message.ts";
import { SuggestedPostInfo } from "./suggested-post.ts";
/**
 * A Telegram message.
 */
export class Message {
    ...
    constructor(raw: tl.TypeMessage, readonly _peers: PeersIndex, 
    /**
    ...
    get groupedIdUnique(): string | null {
        if (!(this.raw._ === 'message' && this.raw.groupedId !== undefined))
            return null;
    ...
    get isAutomaticForward(): boolean {
        ...
        return Boolean(this.chat.type === 'chat'
    ...
    get replyToMessage(): RepliedMessageInfo | null {
        if (this.raw.replyTo?._ !== 'messageReplyHeader')
            return null;
    ...
    get replyToStory(): tl.RawMessageReplyStoryHeader | null {
        if (this.raw.replyTo?._ !== 'messageReplyStoryHeader')
            return null;
    ...
    get suggestedPaid(): 'ton' | 'stars' | null {
        if (this.raw._ !== 'message')
            return null;
        if (this.raw.paidSuggestedPostStars)
            return 'stars';
        if (this.raw.paidSuggestedPostTon)
            return 'ton';
        return null;
    }
