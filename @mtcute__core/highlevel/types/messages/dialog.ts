...
import { assertTypeIsNot, hasValueAtKey } from "../../../utils/type-assertions.ts";
import { makeInspectable } from "../../utils/index.ts";
import { memoizeGetters } from "../../utils/memoize.ts";
import { parsePeer } from "../peers/peer.ts";
import { PeersIndex } from "../peers/peers-index.ts";
import { DraftMessage } from "./draft-message.ts";
 ...
 */
export class Dialog {
    constructor(readonly raw: tl.RawDialog, readonly _peers: PeersIndex, readonly _messages: Map<number, tl.TypeMessage>) { }
    /**
    ...
    static filterFolder(folder: tl.TypeDialogFilter, excludePinned = true): (val: Dialog) => boolean {
        ...
        if (folder._ === 'dialogFilterChatlist') {
            return (dialog) => {
                ...
                if (excludePinned && pinned[peerId])
                    return false;
        ...
        return (dialog) => {
            ...
            if (folder.broadcasts && peer.type === 'chat' && peer.chatType === 'channel')
                return true;
            if (folder.bots && peer.type === 'user' && peer.isBot)
                return true;
            return false;
        };
    }
