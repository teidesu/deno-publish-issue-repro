...
import { RawDocument } from "./document.ts";
...
export class Video extends RawDocument {
    ...
    protected override _fileIdType(): tdFileId.FileType {
        if (this.isRound)
            return tdFileId.FileType.VideoNote;
        if (this.isAnimation)
            return tdFileId.FileType.Animation;
        return tdFileId.FileType.Video;
    }
    constructor(doc: tl.RawDocument, readonly attr: tl.RawDocumentAttributeVideo | tl.RawDocumentAttributeImageSize, readonly media?: tl.RawMessageMediaDocument | undefined) {
        super(doc);
    }
    /**
    ...
    get videoCover(): Photo | null {
        if (!this.media || this.media.videoCover?._ !== 'photo')
            return null;
