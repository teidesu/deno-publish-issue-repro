...
import type { RespondFn, SerializedResult, WorkerCustomMethods, WorkerInboundMessage, WorkerMessageHandler, } from "./protocol.ts";
import { serializeError } from "./errors.ts";
import { DEFAULT_WORKER_ID, deserializeResult, serializeResult, } from "./protocol.ts";
...
export abstract class TelegramWorker<T extends WorkerCustomMethods> {
    ...
    abstract registerWorker(handler: WorkerMessageHandler): [
        RespondFn,
        VoidFunction
    ];
    ...
    constructor(readonly params: TelegramWorkerOptions<T>) {
        ...
        client.onError.add(err => this.broadcast({
            _mtcuteWorkerId: this.workerId,
            type: 'error',
            error: serializeError(err),
        }));
        client.onConnectionState.add(state => this.broadcast({
            _mtcuteWorkerId: this.workerId,
            type: 'conn_state',
            state,
        }));
        ...
        if (client.updates) {
            client.onRawUpdate.add(({ update, peers }) => this.broadcast({
                _mtcuteWorkerId: this.workerId,
                type: 'update',
            ...
            client.onServerUpdate.add(update => this.broadcast({
                _mtcuteWorkerId: this.workerId,
                type: 'server_update',
                update: serializeResult(update),
            }));
    ...
    private onInvoke(msg: Extract<WorkerInboundMessage, {
        type: 'invoke';
    }>, respond: RespondFn) {
        let target: any;
        switch (msg.target) {
