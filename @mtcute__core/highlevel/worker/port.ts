...
import type { RpcCallOptions } from "../../network/network-manager.ts";
...
import type { ClientMessageHandler, SendFn, SomeWorker, WorkerCustomMethods } from "./protocol.ts";
...
import { AppConfigManagerProxy } from "./app-config.ts";
import { deserializeError } from "./errors.ts";
import { WorkerInvoker } from "./invoker.ts";
import { DEFAULT_WORKER_ID, deserializeResult } from "./protocol.ts";
import { TelegramStorageProxy } from "./storage.ts";
...
export abstract class TelegramWorkerPort<Custom extends WorkerCustomMethods> implements ITelegramClient {
    ...
    call<T extends tl.RpcMethod>(message: MustEqual<T, tl.RpcMethod>, params?: RpcCallOptions): Promise<tl.RpcCallReturn[T['_']]> {
        if (params?.abortSignal) {
            ...
            return this._invoker.invokeWithAbort('client', 'call', [message, rest], abortSignal) as Promise<tl.RpcCallReturn[T['_']]>;
        }
        return this._invoker.invoke('client', 'call', [message, params]) as Promise<tl.RpcCallReturn[T['_']]>;
    }
    abstract connectToWorker(worker: SomeWorker, handler: ClientMessageHandler): [
        SendFn,
        () => void
    ];
    ...
    private _onMessage: ClientMessageHandler = (message) => {
        if (message._mtcuteWorkerId !== this.workerId)
            return;
        switch (message.type) {
            case 'log':
