...
import type { RefMessagesService } from "./ref-messages.ts";
import { asyncPool, LruMap, timers } from '@fuman/utils';
import Long from 'long';
import { BaseService } from "../../../storage/service/base.ts";
import { longFromFastString, longToFastString } from "../../../utils/long-utils.ts";
import { getAllPeersFrom, parseMarkedPeerId, toggleChannelIdMark } from "../../../utils/peer-utils.ts";
import { extractUsernames } from "../../utils/peer-utils.ts";
interface CacheItem {
...
export class PeersService extends BaseService {
    ...
    constructor(private options: PeersServiceOptions, private _peers: IPeersRepository, private _refs: RefMessagesService, common: ServiceOptions) {
        super(common);
    ...
    private _writePendingLater() {
        if (this._pendingWritesTimer)
            return;
        this._pendingWritesTimer = timers.setTimeout(this._writePending, this.options.updatesWriteInterval ?? 30000);
    }
    async updatePeersFrom(obj: tl.TlObject | tl.TlObject[]): Promise<boolean> {
        let count = 0;
        let minCount = 0;
        for (const peer of getAllPeersFrom(obj)) {
            if ((peer as Extract<typeof peer, {
                min?: unknown;
            }>).min) {
                minCount += 1;
            }
    ...
    async store(peer: tl.TypeUser | tl.TypeChat): Promise<void> {
        ...
        if ((peer as Extract<typeof peer, {
            ...
            if (existing && !(existing as Extract<typeof existing, {
                min?: unknown;
            }>).min) {
        ...
        if (cached && this.options.updatesWriteInterval !== 0) {
            const oldAccessHash = (cached.peer as Extract<tl.TypeInputPeer, {
                accessHash?: unknown;
            }>).accessHash;
