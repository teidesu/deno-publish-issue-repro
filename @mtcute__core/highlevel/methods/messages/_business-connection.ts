...
import { getBusinessConnection } from "../premium/get-business-connection.ts";
// temporary solution
...
function getDcMap(client: ITelegramClient): LruMap<string, number> {
    const client_ = client as typeof client & {
        [DC_MAP_SYMBOL]?: LruMap<string, number>;
    };
 ...
 * @internal
 */
export async function _maybeInvokeWithBusinessConnection<T extends tl.RpcMethod>(client: ITelegramClient, businessConnectionId: string | undefined, request: MustEqual<T, tl.RpcMethod>, params?: RpcCallOptions): Promise<tl.RpcCallReturn[T['_']]> {
