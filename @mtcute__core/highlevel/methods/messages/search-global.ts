...
import type { ArrayPaginated } from "../../types/index.ts";
import { assertTypeIsNot } from "../../../utils/type-assertions.ts";
import { Message, PeersIndex, SearchFilters } from "../../types/index.ts";
import { makeArrayPaginated, normalizeDate } from "../../utils/index.ts";
// @exported
 ...
 */
export async function searchGlobal(client: ITelegramClient, params?: {
    /**
    ...
    if (!params)
        params = {};
    const { query = '', filter = SearchFilters.Empty, limit = 100, offset: { rate: offsetRate, peer: offsetPeer, id: offsetId } = defaultOffset, onlyChannels, } = params;
