...
import { deleteScheduledMessages } from "./delete-scheduled-messages.ts";
// @exported
 ...
 */
export async function deleteMessagesById(client: ITelegramClient, chatId: InputPeerLike, ids: number[], params?: DeleteMessagesParams): Promise<void> {
 ...
 */
export async function deleteMessages(client: ITelegramClient, messages: Message[], params?: DeleteMessagesParams): Promise<void> {
    if (messages.length === 1) {
        return deleteMessagesById(client, messages[0].chat.inputPeer, [messages[0].id], params);
    }
    const byChat = new Map<number, [
        tl.TypeInputPeer,
        number[]
    ]>();
    const byChatScheduled = new Map<number, [
        tl.TypeInputPeer,
        number[]
    ]>();
