...
import type { TypingStatus } from "../../types/peers/typing-status.ts";
...
import { _mapTypingStatus } from "./send-typing.ts";
...
const TIMER_INTERVAL = 5000; // 5 seconds
/**
 ...
 */
export async function setTyping(client: ITelegramClient, params: {
    ...
    const { peerId, businessConnectionId, threadId, } = params;
    let status = params.status ?? 'typing';
    if (typeof status === 'string')
        status = _mapTypingStatus(status);
