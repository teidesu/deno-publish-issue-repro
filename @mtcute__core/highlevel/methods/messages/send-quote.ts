...
function extractQuote(message: Message, from: number, to: number): TextWithEntities {
    ...
    if (!raw.entities)
        return { text: text.slice(from, to), entities: undefined };
...
export function quoteWithText(client: ITelegramClient, message: Message, params: QuoteParamsFrom<Parameters<typeof sendText>[3]> & {
    /** Text to send */
...
export function quoteWithMedia(client: ITelegramClient, message: Message, params: QuoteParamsFrom<Parameters<typeof sendMedia>[3]> & {
    /** Media to send */
...
export function quoteWithMediaGroup(client: ITelegramClient, message: Message, params: QuoteParamsFrom<Parameters<typeof sendMediaGroup>[3]> & {
    /** Media group to send */
