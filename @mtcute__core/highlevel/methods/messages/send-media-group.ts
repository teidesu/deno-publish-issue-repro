...
import { _normalizeInputMedia } from "../files/normalize-input-media.ts";
 ...
 * @link InputMedia
 */
export async function sendMediaGroup(client: ITelegramClient, chatId: InputPeerLike, medias: (InputMediaLike | string)[], params?: CommonSendParams & {
    /**
    ...
    for (let i = 0; i < medias.length; i++) {
        ...
        const [message, entities] = await _normalizeInputText(client, 
    ...
    const msgs = res.updates
        .filter((u): u is tl.RawUpdateNewMessage | tl.RawUpdateNewChannelMessage | tl.RawUpdateNewScheduledMessage => u._ === 'updateNewMessage' || u._ === 'updateNewChannelMessage' || u._ === 'updateNewScheduledMessage')
