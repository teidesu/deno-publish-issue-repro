...
import { _normalizeQuickReplyShortcut } from "./send-common.ts";
 ...
 */
export async function forwardMessagesById(client: ITelegramClient, params: ForwardMessageOptions & {
    /** Source chat ID, username, phone, `"me"` or `"self"` */
    fromChatId: InputPeerLike;
    /** Message IDs to forward */
    messages: number[];
}): Promise<Message[]> {
    const { messages, toChatId, fromChatId, silent, schedule, forbidForwards, sendAs, noAuthor, noCaption, allowPaidFloodskip, videoTimestamp, toThreadId, toMonoforumPeer, } = params;
 ...
 */
export async function forwardMessages(client: ITelegramClient, params: ForwardMessageOptions & {
    messages: Message[];
}): Promise<Message[]> {
