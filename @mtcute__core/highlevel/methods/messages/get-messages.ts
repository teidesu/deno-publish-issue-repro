 ...
 */
export async function getMessages(client: ITelegramClient, chatId: InputPeerLike, messageIds: MaybeArray<number>, fromReply = false): Promise<(Message | null)[]> {
    ...
    return res.messages.map((msg) => {
        ...
        if (!isChannel) {
            ...
            switch (peer._) {
                case 'inputPeerSelf':
                    if (selfId === undefined)
                        selfId = client.storage.self.getCached()?.userId ?? null;
