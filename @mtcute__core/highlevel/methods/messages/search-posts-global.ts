...
import type { ArrayPaginatedWithMeta } from "../../types/utils.ts";
 ...
 */
export async function searchPostsGlobal(client: ITelegramClient, query: string, params?: {
    ...
    const { offset: { rate: offsetRate, peer: offsetPeer, id: offsetId } = defaultOffset, limit = 100, payStars, } = params ?? {};
    ...
    const next = last
        ...
        : undefined;
    return makeArrayPaginatedWithMeta(msgs, (res as tl.messages.RawMessagesSlice).count ?? msgs.length, { limits: (res as tl.messages.RawMessagesSlice).searchFlood }, next);
}
