...
import { MessageReactions, PeersIndex } from "../../types/index.ts";
 ...
 */
export async function getMessageReactionsById(client: ITelegramClient, chatId: InputPeerLike, messages: number[]): Promise<(MessageReactions | null)[]> {
 ...
 * @returns  Reactions to corresponding messages, or `null` if there are none
 */
export async function getMessageReactions(client: ITelegramClient, messages: Message[]): Promise<(MessageReactions | null)[]> {
    return getMessageReactionsById(client, messages[0].chat.inputPeer, messages.map(it => it.id));
}
