...
import { replyMedia, replyMediaGroup, replyText } from "./send-reply.ts";
 ...
 */
export function commentText(client: ITelegramClient, message: Message, ...params: ParametersSkip2<typeof sendText>): ReturnType<typeof sendText> {
 ...
 *     To check if a post has comments, use {@link Message#replies}.hasComments
 */
export function commentMedia(client: ITelegramClient, message: Message, ...params: ParametersSkip2<typeof sendMedia>): ReturnType<typeof sendMedia> {
    if (message.chat.type !== 'chat' || message.chat.chatType !== 'channel') {
 ...
 *     To check if a post has comments, use {@link Message#replies}.hasComments
 */
export function commentMediaGroup(client: ITelegramClient, message: Message, ...params: ParametersSkip2<typeof sendMediaGroup>): ReturnType<typeof sendMediaGroup> {
    if (message.chat.type !== 'chat' || message.chat.chatType !== 'channel') {
