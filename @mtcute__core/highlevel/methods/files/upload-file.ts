...
import type { UploadedFile, UploadFileLike } from "../../types/index.ts";
...
import { guessFileMime, MIME_TO_EXTENSION } from "../../utils/file-type.ts";
import { determinePartSize, isProbablyPlainText } from "../../utils/file-utils.ts";
 ...
 */
export async function uploadFile(client: ITelegramClient, params: {
    /**
     * Upload file source.
     */
    file: UploadFileLike;
    ...
    client.log.debug('uploading %d bytes file in %d chunks, each %d bytes in %s connection pool of size %d', fileSize, partCount, partSize, connectionKind, connectionPoolSize);
