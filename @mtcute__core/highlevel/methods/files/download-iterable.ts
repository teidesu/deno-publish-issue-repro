import type { ConnectionKind } from "../../../network/network-manager.ts";
...
import { MtArgumentError, MtUnsupportedError } from "../../../types/errors.ts";
import { toggleChannelIdMark } from "../../../utils/peer-utils.ts";
import { FileLocation, MtPeerNotFoundError } from "../../types/index.ts";
import { fileIdToInputFileLocation, fileIdToInputWebFileLocation } from "../../utils/convert-file-id.ts";
import { determinePartSize } from "../../utils/file-utils.ts";
 ...
 */
export async function* downloadAsIterable(client: ITelegramClient, input: FileDownloadLocation, params?: FileDownloadParameters): AsyncIterableIterator<Uint8Array> {
    ...
    client.log.debug('Downloading file of size %d from dc %d using %s connection pool (pool size: %d)', limitBytes, dcId, connectionKind, poolSize);
