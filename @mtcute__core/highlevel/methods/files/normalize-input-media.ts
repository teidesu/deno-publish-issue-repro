...
import type { UploadFileLike } from "../../types/files/utils.ts";
import type { InputMediaLike } from "../../types/media/input-media/types.ts";
import { parseFileId, tdFileId } from '@mtcute/file-id';
import { tl } from '@mtcute/tl';
import Long from 'long';
import { assertTypeIs } from "../../../utils/type-assertions.ts";
import { isUploadedFile } from "../../types/files/uploaded-file.ts";
import { inputTextToTl } from "../../types/misc/entities.ts";
import { fileIdToInputDocument, fileIdToInputPhoto } from "../../utils/convert-file-id.ts";
import { normalizeDate } from "../../utils/misc-utils.ts";
import { encodeWaveform } from "../../utils/voice-utils.ts";
import { _normalizeInputText } from "../misc/normalize-text.ts";
import { resolvePeer } from "../users/resolve-peer.ts";
import { _normalizeInputFile } from "./normalize-input-file.ts";
import { uploadFile } from "./upload-file.ts";
/**
 * Normalize an {@link InputMediaLike} to `InputMedia`,
 * uploading the file if needed.
 */
export async function _normalizeInputMedia(client: ITelegramClient, media: InputMediaLike, params: {
    progressCallback?: (uploaded: number, total: number) => void;
    uploadPeer?: tl.TypeInputPeer;
    abortSignal?: AbortSignal;
    businessConnectionId?: string;
} = {}, uploadMedia = false): Promise<tl.TypeInputMedia> {
    ...
    if (media.type === 'poll' || media.type === 'quiz') {
        const answers: tl.TypePollAnswer[] = media.answers.map((ans, idx) => {
            if (typeof ans === 'object' && tl.isAnyPollAnswer(ans))
                return ans;
            return {
                _: 'pollAnswer',
    ...
    if (tdFileId.isFileIdLike(input)) {
        ...
        if (typeof input === 'string' && input.match(/^https?:\/\//)) {
            ...
            if (parsed.location._ === 'photo') {
                ...
                return uploadMediaIfNeeded({
                    _: parsed.type === tdFileId.FileType.Photo
