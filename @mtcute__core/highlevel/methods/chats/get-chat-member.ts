...
import { ChatMember, MtInvalidPeerTypeError, PeersIndex } from "../../types/index.ts";
import { isInputPeerChannel, isInputPeerChat, isInputPeerUser, toInputChannel } from "../../utils/peer-utils.ts";
 ...
 */
export async function getChatMember(client: ITelegramClient, params: {
    /** Chat ID or username */
    ...
    if (isInputPeerChat(chat)) {
        ...
        for (const m of members) {
            if ((user._ === 'inputPeerSelf' && (peers.user(m.userId) as tl.RawUser).self)
