...
import type { BatchedQuery } from "../../utils/query-batcher.ts";
import { tl } from '@mtcute/tl';
import { MtArgumentError } from "../../../types/errors.ts";
import { isInputPeerChannel, isInputPeerChat, isInputPeerUser, toInputChannel, toInputUser, } from "../../utils/peer-utils.ts";
import { batchedQuery } from "../../utils/query-batcher.ts";
/** @internal */
export const _getUsersBatched: BatchedQuery<tl.TypeInputUser, tl.TypeUser> = batchedQuery({
    fetch: (client, items) => client
        .call({
        _: 'users.getUsers',
        id: items,
...
/** @internal */
export const _getChannelsBatched: BatchedQuery<tl.TypeInputChannel, tl.RawChannel | tl.RawChannelForbidden> = batchedQuery({
    fetch: (client, items) => client
        .call({
        _: 'channels.getChannels',
        id: items,
    })
        .then(res => res.chats.filter((it): it is tl.RawChannel | tl.RawChannelForbidden => it._ === 'channel' || it._ === 'channelForbidden')),
    inputKey: (id) => {
        switch (id._) {
            case 'inputChannel':
...
/** @internal */
export function _getRawPeerBatched(client: ITelegramClient, peer: tl.TypeInputPeer): Promise<tl.TypeUser | tl.TypeChat | null> {
