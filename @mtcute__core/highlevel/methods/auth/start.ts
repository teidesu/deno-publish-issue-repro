...
import type { SentCode } from "../../types/auth/sent-code.ts";
import type { MaybeDynamic } from "../../types/utils.ts";
import type { InputStringSessionData } from "../../utils/string-session.ts";
import { tl } from '@mtcute/tl';
import { MtArgumentError, MtcuteError } from "../../../types/errors.ts";
import { User } from "../../types/peers/user.ts";
import { normalizePhoneNumber, resolveMaybeDynamic } from "../../utils/misc-utils.ts";
import { getMe } from "../users/get-me.ts";
import { checkPassword } from "./check-password.ts";
import { logOut } from "./log-out.ts";
import { resendCode } from "./resend-code.ts";
import { sendCode } from "./send-code.ts";
import { signInBot } from "./sign-in-bot.ts";
import { signInQr } from "./sign-in-qr.ts";
 ...
 */
export async function start(client: ITelegramClient, params: {
    /**
    ...
    if (sentCode) {
        ...
        for (;;) {
            ...
            if (!code)
                throw new tl.RpcError(400, 'PHONE_CODE_EMPTY');
            try {
