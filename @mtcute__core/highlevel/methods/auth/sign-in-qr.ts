...
import type { MaybeDynamic } from "../../types/index.ts";
...
import { resolveMaybeDynamic } from "../../utils/misc-utils.ts";
 ...
 */
export async function signInQr(client: ITelegramClient, params: {
    /**
        ...
        loop: while (true) {
            ...
            switch (res._) {
                case 'auth.loginToken':
                    onUrlUpdated(`tg://login?token=${base64.encode(res.token, true)}`, new Date(res.expires * 1000));
                    waiter = new Deferred();
                    await Promise.race([
                        waiter.promise,
