...
import type { getSavedStarGifts } from "./get-saved-star-gifts.ts";
import { PeersIndex } from "../../types/index.ts";
import { SavedStarGift } from "../../types/premium/saved-star-gift.ts";
import { resolvePeer } from "../users/resolve-peer.ts";
/**
 * Iterate over saved star gifts of a user,
 * wrapper over {@link getSavedStarGifts}
 */
export async function* iterSavedStarGifts(client: ITelegramClient, params: Parameters<typeof getSavedStarGifts>[1] & {
    /**
    ...
    const { owner, offset: offsetInitial = '', chunkSize = 100, limit = Infinity, ...rest } = params;
