...
import type { InputDialogFolder } from "../../types/index.ts";
import Long from 'long';
import { MtArgumentError } from "../../../types/errors.ts";
import { Dialog } from "../../types/index.ts";
import { normalizeDate } from "../../utils/misc-utils.ts";
import { _normalizeInputFolder } from "./get-folders.ts";
/**
 ...
 */
export async function* iterDialogs(client: ITelegramClient, params?: {
    /**
    ...
    if (pinned === 'only') {
        ...
        if (res)
            yield* Dialog.parseTlDialogs(res, limit);
        return;
    }
    let current = 0;
    if (localFilters?.pinnedPeers.length
        && pinned === 'include'
        && offsetId === 0
        && offsetDate === 0
