...
import type { PartialExcept } from "../../../types/utils.ts";
import type { ITelegramClient } from "../../client.types.ts";
import { assertTrue } from "../../../utils/type-assertions.ts";
import { getFolders } from "./get-folders.ts";
/**
 ...
 */
export async function createFolder(client: ITelegramClient, folder: PartialExcept<tl.RawDialogFilter, 'title'>): Promise<tl.RawDialogFilter> {
    let id = folder.id;
    if (!id) {
        ...
        old.filters.forEach((it) => {
            if (it._ === 'dialogFilter' && it.id > max)
                max = it.id;
        });
        id = max + 1;
    }
