 ...
 */
export async function resolvePeerMany<T extends tl.TypeInputPeer | tl.TypeInputUser | tl.TypeInputChannel>(client: ITelegramClient, peerIds: InputPeerLike[], normalizer: (obj: tl.TypeInputPeer) => T | null): Promise<T[]>;
/**
 * Get multiple `InputPeer`s at once.
 *
 ...
 * @param peerIds  Peer Ids
 */
export async function resolvePeerMany(client: ITelegramClient, peerIds: InputPeerLike[]): Promise<(tl.TypeInputPeer | null)[]>;
/**
 * @internal
 */
export async function resolvePeerMany(client: ITelegramClient, peerIds: InputPeerLike[], normalizer?: (obj: tl.TypeInputPeer) => tl.TypeInputPeer | tl.TypeInputUser | tl.TypeInputChannel | null): Promise<(tl.TypeInputPeer | tl.TypeInputUser | tl.TypeInputChannel | null)[]> {
    ...
    if (peerIds.length < limit) {
        ...
        const res = await Promise.all(peerIds.map(it => resolvePeer(client, it).catch((e) => {
