...
import { MtArgumentError, MtTypeAssertionError } from "../../../types/errors.ts";
import { getMarkedPeerId, parseMarkedPeerId, toggleChannelIdMark } from "../../../utils/peer-utils.ts";
import { MtPeerNotFoundError } from "../../types/errors.ts";
import { extractUsernames, toInputChannel, toInputPeer, toInputUser } from "../../utils/peer-utils.ts";
import { _getChannelsBatched, _getUsersBatched } from "../chats/batched-queries.ts";
import { resolvePhoneNumber } from "./resolve-phone-number.ts";
...
export function _handleContactsResolvedPeer(peerId: string, res: tl.contacts.TypeResolvedPeer): tl.TypeInputPeer {
    if (res.peer._ === 'peerUser') {
        ...
        if (found && found._ === 'user') {
            if (!found.accessHash) {
                ...
                throw new MtPeerNotFoundError(`Peer (user) with username ${peerId} was found, but it has no access hash`);
            }
            return {
                _: 'inputPeerUser',
                userId: found.id,
 ...
 */
export async function resolvePeer(client: ITelegramClient, peerId: InputPeerLike, force = false): Promise<tl.TypeInputPeer> {
    ...
    if (typeof peerId === 'string') {
        ...
        if (peerId.match(/^\d/)) {
            throw new MtArgumentError(`Invalid username: ${peerId}. If you wanted to resolve by user ID, pass a number instead. If you wanted to resolve by phone number, use resolvePhoneNumber instead.`);
        }
        if (!force) {
    ...
    switch (peerType) {
        case 'user': {
            try {
                ...
                if (!tl.RpcError.is(e, 'USER_INVALID'))
                    throw e;
            }
            break;
        }
        case 'channel': {
            try {
                ...
                if (!tl.RpcError.is(e, 'CHANNEL_INVALID'))
                    throw e;
            }
            break;
        }
 ...
 */
export async function resolveUser(client: ITelegramClient, peerId: InputPeerLike, force = false): Promise<tl.TypeInputUser> {
 ...
 */
export async function resolveChannel(client: ITelegramClient, peerId: InputPeerLike, force = false): Promise<tl.TypeInputChannel> {
