...
import type { AppConfigSchema } from "../types/misc/app-config.ts";
import { asNonNull, AsyncResource } from '@fuman/utils';
import { MtTypeAssertionError } from "../../types/errors.ts";
import { tlJsonToJson } from "../../utils/tl-json.ts";
export class AppConfigManager {
    ...
    async get(): Promise<AppConfigSchema> {
        if (!this._resource.isStale && this._object)
            return this._object;
    ...
    async getField<K extends keyof AppConfigSchema>(field: K, fallback: NonNullable<AppConfigSchema[K]>): Promise<NonNullable<AppConfigSchema[K]>>;
    async getField<K extends keyof AppConfigSchema>(field: K, fallback?: NonNullable<AppConfigSchema[K]>): Promise<AppConfigSchema[K]> {
