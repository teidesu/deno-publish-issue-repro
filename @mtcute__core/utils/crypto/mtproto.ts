import type { ICryptoProvider, IEncryptionScheme } from "./abstract.ts";
 ...
 */
export function generateKeyAndIvFromNonce(crypto: ICryptoProvider, serverNonce: Uint8Array, newNonce: Uint8Array): [
    Uint8Array,
    Uint8Array
] {
 ...
 */
export function createAesIgeForMessage(crypto: ICryptoProvider, authKey: Uint8Array, messageKey: Uint8Array, client: boolean): IEncryptionScheme {
    const x = client ? 0 : 8;
 ...
 * @param client  Whether this is a client to server message
 */
export function createAesIgeForMessageOld(crypto: ICryptoProvider, authKey: Uint8Array, messageKey: Uint8Array, client: boolean): IEncryptionScheme {
    const x = client ? 0 : 8;
    const sha1a = crypto.sha1(u8.concat2(messageKey, authKey.subarray(x, 32 + x)));
    const sha1b = crypto.sha1(u8.concat3(authKey.subarray(32 + x, 48 + x), messageKey, authKey.subarray(48 + x, 64 + x)));
