...
import type { ICryptoProvider } from "./abstract.ts";
import { bigint, u8, utf8 } from '@fuman/utils';
import { MtSecurityError, MtUnsupportedError } from "../../types/errors.ts";
import { assertTypeIs } from "../type-assertions.ts";
/**
 ...
 * @param salt2  Salt 2
 */
export async function computePasswordHash(crypto: ICryptoProvider, password: Uint8Array, salt1: Uint8Array, salt2: Uint8Array): Promise<Uint8Array> {
 ...
 * @param password  Password
 */
export async function computeNewPasswordHash(crypto: ICryptoProvider, algo: tl.TypePasswordKdfAlgo, password: string): Promise<Uint8Array> {
 ...
 */
export async function computeSrpParams(crypto: ICryptoProvider, request: tl.account.RawPassword, password: string): Promise<tl.RawInputCheckPasswordSRP> {
    // nice naming thx durov
    if (!request.currentAlgo
