import type { ISqliteDatabase, ISqliteStatement } from "./types.ts";
import { BaseStorageDriver } from "../driver.ts";
...
export abstract class BaseSqliteStorageDriver extends BaseStorageDriver {
    ...
    _initialize(): void {
        ...
        const writeVersion = this.db.prepare(`insert or replace into ${MIGRATIONS_TABLE_NAME} (repo, version) values (?, ?)`);
        ...
        for (const repo of this._migrations.keys()) {
            const res = getVersion.get(repo) as {
                version: number;
            } | undefined;
