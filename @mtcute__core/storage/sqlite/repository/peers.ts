import type { IPeersRepository } from "../../../highlevel/storage/repository/peers.ts";
import type { BaseSqliteStorageDriver } from "../driver.ts";
import type { ISqliteStatement } from "../types.ts";
import { MtcuteError } from "../../../types/errors.ts";
interface PeerDto {
    id: number;
    hash: string;
    isMin: 1 | 0;
...
export class SqlitePeersRepository implements IPeersRepository {
    ...
    constructor(readonly _driver: BaseSqliteStorageDriver) {
        ...
        _driver.onLoad((db) => {
            this._loaded = true;
            this._store = db.prepare('insert or replace into peers (id, hash, isMin, usernames, updated, phone, complete) values (?, ?, ?, ?, ?, ?, ?)');
            this._getById = db.prepare('select * from peers where id = ?');
            this._getByUsername = db.prepare('select * from peers where exists (select 1 from json_each(usernames) where value = ?) and isMin = false');
