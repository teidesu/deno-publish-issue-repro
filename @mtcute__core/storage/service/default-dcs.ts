...
import { parseBasicDcOption, serializeBasicDcOption } from "../../utils/dcs.ts";
...
export class DefaultDcsService extends BaseService {
    constructor(readonly _kv: IKeyValueRepository, opts: ServiceOptions) {
        super(opts);
    }
    private _cached?: DcOptions;
    async store(dcs: DcOptions): Promise<void> {
        if (this._cached) {
            if (this._cached.main === dcs.main
