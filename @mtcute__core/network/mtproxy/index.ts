...
import type { BasicDcOption, ICryptoProvider, Logger } from "../../utils/index.ts";
import type { IPacketCodec, ITelegramConnection, MtProxyInfo, TelegramTransport } from "../transports/index.ts";
...
import { IntermediatePacketCodec, ObfuscatedPacketCodec, PaddedIntermediatePacketCodec } from "../transports/index.ts";
import { FakeTlsPacketCodec, generateFakeTlsHeader } from "./_fake-tls.ts";
/**
...
export abstract class BaseMtProxyTransport implements TelegramTransport {
    ...
    packetCodec(dc: BasicDcOption): IPacketCodec {
        ...
        return new FakeTlsPacketCodec(new ObfuscatedPacketCodec(new PaddedIntermediatePacketCodec(), proxy));
    }
    ...
    private async _handleConnectFakeTls(conn: ITcpConnection): Promise<void> {
        ...
        const hash = await this._crypto.hmacSha256(u8.concat([
            helloRand,
            respBuf.slice(0, 11),
            u8.alloc(32),
            respBuf.slice(11 + 32),
