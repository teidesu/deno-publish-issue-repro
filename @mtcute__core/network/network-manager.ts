...
import type { ICorePlatform } from "../types/platform.ts";
import type { DcOptions, ICryptoProvider, Logger } from "../utils/index.ts";
import type { ConfigManager } from "./config-manager.ts";
import type { SessionConnectionParams } from "./session-connection.ts";
import type { TelegramTransport } from "./transports/abstract.ts";
import { defaultReconnectionStrategy } from '@fuman/net';
import { asNonNull, composeMiddlewares, Deferred } from '@fuman/utils';
import { MtArgumentError, MtcuteError, MtUnsupportedError } from "../types/index.ts";
import { assertTypeIs, isTlRpcError } from "../utils/type-assertions.ts";
import { basic as defaultMiddlewares } from "./middlewares/default.ts";
import { MultiSessionConnection } from "./multi-session-connection.ts";
import { ServerSaltManager } from "./server-salt.ts";
...
export class DcConnectionManager {
    ...
    public isPrimary = false) {
        ...
        const baseConnectionParams = (): SessionConnectionParams => ({
            ...
            inactivityTimeout: managerParams.inactivityTimeout ?? 60000,
            ...
            pingInterval: managerParams.pingInterval ?? 60000,
        });
        ...
        this.upload = new MultiSessionConnection(baseConnectionParams(), this.manager._connectionCount('upload', this.dcId, managerParams.isPremium), this._log, 'UPLOAD');
        this.download = new MultiSessionConnection(baseConnectionParams(), this.manager._connectionCount('download', this.dcId, managerParams.isPremium), this._log, 'DOWNLOAD');
        this.downloadSmall = new MultiSessionConnection(baseConnectionParams(), this.manager._connectionCount('downloadSmall', this.dcId, managerParams.isPremium), this._log, 'DOWNLOAD_SMALL');
    ...
    private _setupMulti(kind: ConnectionKind): void {
        ...
        connection.onFutureSalts.add((salts: mtp.RawMt_future_salt[]) => {
            ...
            Promise.resolve(this.manager._storage.salts.store(this.dcId, salts)).catch((e: Error) => this.manager.params.emitError(e));
        });
    ...
    setIsPrimary(isPrimary: boolean): void {
        ...
        if (isPrimary) {
            ...
            this.main.setInactivityTimeout(this.manager.params.inactivityTimeout ?? 60000);
        }
    }
    ...
    async loadKeys(forcePfs = false): Promise<boolean> {
        ...
        if (this.manager.params.usePfs || forcePfs) {
            ...
            await Promise.all(Array.from({ length: this.main.getCount() }, async (_, i) => {
...
export class NetworkManager {
    ...
    notifyLoggedIn(auth: tl.auth.TypeAuthorization | tl.RawUser): tl.RawUser {
        if (auth._ === 'auth.authorizationSignUpRequired') {
            throw new MtUnsupportedError('Signup is no longer supported by Telegram for non-official clients. Please use your mobile device to sign up.');
        }
        let user: tl.RawUser;
    ...
    private _composeCall = (middlewares?: Middleware<RpcCallMiddlewareContext, unknown>[]) => {
        ...
        return async <T extends tl.RpcMethod>(message: T, params?: RpcCallOptions): Promise<tl.RpcCallReturn[T['_']]> => composed({
            request: message,
            manager: this,
            params,
        });
    };
    private _call = async <T extends tl.RpcMethod>(message: T, params?: RpcCallOptions): Promise<tl.RpcCallReturn[T['_']] | mtp.RawMt_rpc_error> => {
        if (!this._primaryDc) {
