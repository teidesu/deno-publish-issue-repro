...
import { MtcuteError } from "../types/index.ts";
import { compareLongs, getRandomInt, LongMap, LongSet, randomLong, SortedArray, } from "../utils/index.ts";
import { AuthKey } from "./auth-key.ts";
...
export class MtprotoSession {
    ...
    constructor(readonly _crypto: ICryptoProvider, readonly log: Logger, readonly _readerMap: TlReaderMap, readonly _writerMap: TlWriterMap, readonly _salts: ServerSaltManager) {
    ...
    get hasPendingMessages(): boolean {
        return Boolean(this.queuedRpc.length
    ...
    decryptMessage(data: Uint8Array, callback: Parameters<AuthKey['decryptMessage']>[2]): void {
        ...
        if (this._authKey.match(authKeyId)) {
            ...
            this.log.warn('received message with unknown authKey = %h (expected %h or %h or %h)', authKeyId, this._authKey.id, this._authKeyTemp.id, this._authKeyTempSecondary.id);
            return;
        }
        return key.decryptMessage(data, this._sessionId, callback);
    }
    writeMessage(writer: TlBinaryWriter, content: tl.TlObject | mtp.TlObject | Uint8Array, isContentRelated = true): Long {
        ...
        if (ArrayBuffer.isView(content))
            ...
            writer.object(content as tl.TlObject);
        return messageId;
    }
