...
import type { ICryptoProvider, Logger } from "../utils/index.ts";
import type { PendingMessage, PendingRpc } from "./mtproto-session.ts";
import type { PersistentConnectionParams } from "./persistent-connection.ts";
import type { ServerSaltManager } from "./server-salt.ts";
...
import { MtArgumentError, MtcuteError, MtTimeoutError } from "../types/index.ts";
import { createAesIgeForMessageOld } from "../utils/crypto/mtproto.ts";
import { EarlyTimer, longFromBuffer, randomLong, removeFromLongArray, } from "../utils/index.ts";
import { doAuthorization } from "./authorization.ts";
import { MtprotoSession } from "./mtproto-session.ts";
import { PersistentConnection } from "./persistent-connection.ts";
import { TransportError } from "./transports/abstract.ts";
...
export class SessionConnection extends PersistentConnection {
    ...
    readonly onTmpKeyChange: Emitter<[
        Uint8Array,
        number
    ] | null> = new Emitter();
    ...
    constructor(params: SessionConnectionParams, log: Logger) {
        ...
        this._session = new MtprotoSession(params.crypto, log.create('session'), params.readerMap, params.writerMap, params.salts);
    ...
    protected onConnectionUsable(): void {
        if (this.params.withUpdates) {
            ...
            this.sendRpc({ _: 'updates.getState' }).catch((err: any) => {
                if (this._destroyed || tl.RpcError.is(err, 'AUTH_KEY_UNREGISTERED'))
    ...
    private _authorizePfs(background = false): void {
        ...
        doAuthorization(this, this._crypto, TEMP_AUTH_KEY_EXPIRY)
            ...
            const requestEncrypted = tempKey.encryptMessage(reqWriter.result(), tempServerSalt, this._session._sessionId);
    ...
    private _handleRawMessage(messageId: Long, seqNo: number, message: TlBinaryReader): void {
        ...
        if (objectId === GZIP_PACKED_ID) {
            return this._handleRawMessage(messageId, seqNo, new TlBinaryReader(this._readerMap, this._crypto.gunzip(message.bytes())));
        }
    ...
    private _flush(): void {
        if (this._disconnectedManually
            ...
            this.log.debug('skipping flush, connection is not usable (offline = %b, auth key ready = %b, pfs binding pending = %b, 429 timeout = %b)', this._disconnectedManually, this._session._authKey.ready, this._isPfsBindingPending, this._session.current429Timeout);
    ...
    private _doFlush(): void {
        ...
        const otherPendings: Exclude<PendingMessage, {
