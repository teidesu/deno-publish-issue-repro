...
import { MtTimeoutError } from "../../types/errors.ts";
import { combineAbortSignals } from "../../utils/abort-signal.ts";
import { sleepWithAbort } from "../../utils/misc-utils.ts";
import { isTlRpcError } from "../../utils/type-assertions.ts";
    ...
    return async (ctx, next) => {
        ...
        for (let i = 0; i <= numRetries; i++) {
            ...
            if (!CLIENT_ERRORS.has(res.errorCode)) {
                if (exceptErrorsSet?.has(res.errorMessage))
                    return res;
                lastError = res;
                ...
                ctx.manager._log.warn('Telegram is having internal issues: %d:%s, retrying in %ds', res.errorCode, res.errorMessage, waitSeconds);
                if (waitSeconds > 0) {
                    await sleepWithAbort(waitSeconds * 1000, combineAbortSignals(ctx.manager.params.stopSignal, ctx.params?.abortSignal));
                }
                continue;
            }
            return res;
