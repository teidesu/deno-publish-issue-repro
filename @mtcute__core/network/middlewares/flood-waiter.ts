...
import type { RpcCallMiddleware, RpcCallMiddlewareContext } from "../network-manager.ts";
...
export function floodWaiter(options: FloodWaiterOptions): RpcCallMiddleware {
    const { maxWait = 10000, maxRetries = 5, store = true, minStoredWait = 2000, onBeforeWait, } = options;
        ...
        for (let i = 0; i <= maxRetries; i++) {
            ...
            if (err.startsWith('FLOOD_WAIT_')
                ...
                if (ms <= floodSleepThreshold) {
                    ...
                    await sleepWithAbort(ms, combineAbortSignals(ctx.manager.params.stopSignal, ctx.params?.abortSignal));
                    continue;
                }
            }
            return res;
