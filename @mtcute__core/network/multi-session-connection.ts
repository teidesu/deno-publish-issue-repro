...
import type { TelegramTransport } from "./transports/index.ts";
import { Deferred, Emitter, unknownToError } from '@fuman/utils';
import { SessionConnection } from "./session-connection.ts";
export class MultiSessionConnection {
    ...
    private _updateConnections(connect = false): void {
        ...
        for (let i = this._connections.length; i < this._count; i++) {
            ...
            conn.onTmpKeyChange.add(event => this.onTmpKeyChange.emit(event === null ? [i, null, 0] : [i, event[0], event[1]]));
            ...
            if (this._authKey !== null)
                conn._session._authKey.setup(this._authKey);
