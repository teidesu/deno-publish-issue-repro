...
import type { SessionConnection } from "./session-connection.ts";
import { bigint, typed, u8 } from '@fuman/utils';
import { mtp } from '@mtcute/tl';
import { TlBinaryReader, TlBinaryWriter, TlSerializationCounter } from '@mtcute/tl-runtime';
import Long from 'long';
import { MtArgumentError, MtSecurityError, MtTypeAssertionError } from "../types/index.ts";
import { findKeyByFingerprints } from "../utils/crypto/keys.ts";
import { millerRabin } from "../utils/crypto/miller-rabin.ts";
import { generateKeyAndIvFromNonce } from "../utils/crypto/mtproto.ts";
import { mtpAssertTypeIs } from "../utils/type-assertions.ts";
    ...
    switch (g) {
        ...
        case 7: {
            ...
            if (mod !== 3n && mod !== 5n && mod !== 6n) {
                throw new MtSecurityError('Step 3: ivalid g - dh_prime mod 7 != 3 && dh_prime mod 7 != 5 && dh_prime mod 7 != 6');
            }
            break;
        }
        default:
...
function rsaEncrypt(data: Uint8Array, crypto: ICryptoProvider, key: TlPublicKey): Uint8Array {
    const toEncrypt = u8.concat3(crypto.sha1(data), data, 
    // sha1 is always 20 bytes, so we're left with 255 - 20 - x padding
    crypto.randomBytes(235 - data.length));
    const encryptedBigInt = bigint.modPowBinary(bigint.fromBytes(toEncrypt), BigInt(`0x${key.exponent}`), BigInt(`0x${key.modulus}`));
 ...
 */
export async function doAuthorization(connection: SessionConnection, crypto: ICryptoProvider, expiresIn?: number): Promise<[
    Uint8Array,
    Long,
    number
]> {
    ...
    async function readNext(): Promise<mtp.TlObject> {
        const res = TlBinaryReader.deserializeObject<mtp.TlObject>(readerMap, await connection.waitForUnencryptedMessage(), 20);
    ...
    if (!publicKey) {
        throw new MtSecurityError(`Step 2: Could not find server public key with any of these fingerprints: ${serverKeys.join(', ')}`);
    }
