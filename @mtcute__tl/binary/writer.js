...
'stories.sendStory':function(w,v){w.uint(1937752812);var flags=0;var _caption=v.caption!==undefined;if(_caption)flags|=1;var _entities=v.entities&&v.entities.length;if(_entities)flags|=2;if(v.pinned===true)flags|=4;var _period=v.period!==undefined;if(_period)flags|=8;if(v.noforwards===true)flags|=16;var _mediaAreas=v.mediaAreas&&v.mediaAreas.length;if(_mediaAreas)flags|=32;var _fwdFromId=v.fwdFromId!==undefined;var _fwdFromStory=v.fwdFromStory!==undefined;var _flags_6=_fwdFromId||_fwdFromStory;if(_flags_6)flags|=64;if(v.fwdModified===true)flags|=128;var _albums=v.albums&&v.albums.length;if(_albums)flags|=256;w.uint(flags);w.object(h(v,'peer'));w.object(h(v,'media'));if(_mediaAreas)w.vector(w.object,v.mediaAreas);if(_caption)w.string(v.caption);if(_entities)w.vector(w.object,v.entities);w.vector(w.object,h(v,'privacyRules'));w.long(h(v,'randomId'));if(_period)w.int(v.period);if(_flags_6)w.object(v.fwdFromId);if(_flags_6)w.int(v.fwdFromStory);if(_albums)w.vector(w.int,v.albums);},
